...
{
	//assuming {l0, l1, r1, k} as xreal in that order
	double l0 = xreal[0], l1 = xreal[1], r1 = xreal[2], k = xreal[3];
	printf("\nl1 = %f, r1 = %f, l0 = %f, k = %f\n", l1, r1, l0, k);
	float pen = std::numeric_limits<float>::max();
//	std::cout<<"checking in workspace"<<std::endl;
	bool cond = inWorkspace(l1, r1, l0, k);
//	std::cout<<"checked in workspace"<<std::endl;
	MatrixXd objs(3, 1);
	if(cond==true){		
		//Get the objective values at the given configuration
//		std::cout<<"finding minrad"<<std::endl;
		float minr = minrad(l1, r1, l0, k);
//		std::cout<<"found minrad"<<std::endl;
//		std::cout<<"finding objective"<<std::endl;
		objs = objective(l1, r1, l0, k, minr);
//		std::cout<<"found objective"<<std::endl;
		//obj[0] = objs(0);
		obj[0] = objs(0);
		obj[1] = -objs(1);
		return;
		...
		//Since the problem does only minimization, when point is outside workspace give function values as inf
		obj[0] = pen;
		obj[1] = pen;
		//obj[2] = pen;
	return;
